...
// Convenience methods that combine factory creation and client creation

// NewDatabaseClientFromEnv creates a database client directly from environment variables
func NewDatabaseClientFromEnv(ctx context.Context) (client.DatabaseClient, error) {
	factory, err := NewClientFactoryFromEnv()
	if err != nil {
		return nil, err
	}

	return factory.CreateDatabaseClient(ctx)
}

// NewCollectionClientFromEnv creates a collection client directly from environment variables
func NewCollectionClientFromEnv(ctx context.Context) (client.CollectionClient, error) {
	factory, err := NewClientFactoryFromEnv()
	if err != nil {
		return nil, err
	}

	return factory.CreateCollectionClient(ctx)
}

// NewChangeStreamWatcherFromEnv creates a change stream watcher directly from environment variables
func NewChangeStreamWatcherFromEnv(
	ctx context.Context,
	clientName string,
	pipeline interface{},
) (client.ChangeStreamWatcher, error) {
...
// CreateLegacyMongoDBConfig creates a legacy MongoDBConfig for modules that haven't been refactored yet
func CreateLegacyMongoDBConfig() (client.MongoDBConfig, error) {
	dbConfig, err := config.NewDatabaseConfigFromEnv()
	if err != nil {
		return client.MongoDBConfig{}, datastore.NewConfigurationError(
			datastore.ProviderMongoDB,
	...
	return client.ConvertToMongoDBConfig(dbConfig), nil
}

// CreateLegacyMongoDBConfigWithCertPath creates a legacy MongoDBConfig with custom certificate path
func CreateLegacyMongoDBConfigWithCertPath(certMountPath string) (client.MongoDBConfig, error) {
	dbConfig, err := config.NewDatabaseConfigFromEnvWithDefaults(certMountPath)
	if err != nil {
		return client.MongoDBConfig{}, datastore.NewConfigurationError(
			datastore.ProviderMongoDB,
