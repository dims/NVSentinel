...
// InsertHealthEvents inserts health events
func (h *MongoHealthEventStore) InsertHealthEvents(ctx context.Context, events *datastore.HealthEventWithStatus) error {
	// Use the existing upsert functionality
	filter := map[string]interface{}{
		"healthevent.nodename": events.HealthEvent,
	}

	_, err := h.databaseClient.UpsertDocument(ctx, filter, events)
	if err != nil {
		return datastore.NewInsertError(
			datastore.ProviderMongoDB,
			"failed to insert health events",
			err,
		).WithMetadata("operation", "upsert").WithMetadata("filter", filter)
	}
