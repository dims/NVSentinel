...
func (p *PostgreSQLHealthEventStore) InsertHealthEventsWithIndexFields(
	...
	slog.Info("[PostgreSQL-DEBUG] InsertHealthEventsWithIndexFields ENTRY",
	...
	slog.Info("[PostgreSQL-DEBUG] Calling insertHealthEventRecord",
		"nodeName", indexFields.nodeName)
...
func (p *PostgreSQLHealthEventStore) extractIndexFields(
	...
	slog.Info("[PostgreSQL-DEBUG] extractIndexFields ENTRY",
		"healthEventType", fmt.Sprintf("%T", eventWithStatus.HealthEvent))
	...
	if protoEvent, ok := eventWithStatus.HealthEvent.(*protos.HealthEvent); ok {
		slog.Info("[PostgreSQL-DEBUG] Successfully type-asserted to *protos.HealthEvent")
		...
		slog.Info("[PostgreSQL-DEBUG] Successfully extracted fields from protobuf",
...
func (p *PostgreSQLHealthEventStore) extractFromMap(healthEvent interface{}) healthEventIndexFields {
	...
	if !ok {
		slog.Warn("[PostgreSQL-DEBUG] Failed to extract fields - type assertion failed",
			"actualType", fmt.Sprintf("%T", healthEvent))

		return fields
	}

	slog.Info("[PostgreSQL-DEBUG] Using map interface fallback")
	...
	slog.Info("[PostgreSQL-DEBUG] Extracted fields from map",
		"nodeName", fields.nodeName,
		"eventType", fields.eventType)
