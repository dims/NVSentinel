...
func CreateChangeStreamWatcher(
	...
	// This would typically be passed through context or stored in the datastore
	// For now, we'll need to infer it or require it in the config
...
// GetRegisteredProviders returns a list of all registered watcher providers
func GetRegisteredProviders() []datastore.DataStoreProvider {
	watcherMutex.RLock()
	defer watcherMutex.RUnlock()

	providers := make([]datastore.DataStoreProvider, 0, len(watcherFactories))
	for provider := range watcherFactories {
...
// DefaultWatcherConfig creates a default configuration for watching health events
...
// HealthEventsWatcherConfig creates a configuration optimized for watching health events
func HealthEventsWatcherConfig(pipeline datastore.Pipeline) WatcherConfig {
