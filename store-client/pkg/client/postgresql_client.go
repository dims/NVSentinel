...
func (c *PostgreSQLClient) buildExprValue(value interface{}) (string, error) {
	...
	case string:
		...
		return "", datastore.NewValidationError(
			datastore.ProviderPostgreSQL,
			fmt.Sprintf("unsupported string value in expression: %s", v),
			nil,
		)
	case map[string]interface{}:
...
func (c *PostgreSQLClient) buildJSONPathWithCast(fieldPath string) string {
	...
	if len(parts) == 1 {
		...
		return fmt.Sprintf("(%s->>'%s')::bigint", jsonbDocumentColumn, parts[0])
	}
	...
	for i, part := range parts {
		if i == len(parts)-1 {
			...
			path = fmt.Sprintf("(%s->>'%s')::bigint", path, part)
		} else {
...
func (c *PostgreSQLClient) buildJSONPath(fieldPath string) string {
	...
	if len(parts) == 1 {
		...
		return fmt.Sprintf("%s->>'%s'", jsonbDocumentColumn, parts[0])
	}
...
func (b *aggregationQueryBuilder) buildFinalQuery() string {
	...
	if b.isCount {
		...
		return fmt.Sprintf("SELECT '1' as id, jsonb_build_object('%s', COUNT(*)) as document FROM (%s) as subq",
