...
func startEventWatcher(ctx context.Context, components *initializer.Components, criticalError chan<- error) {
	go func() {
		// Check if event watcher exists (nil for PostgreSQL)
		if components.EventWatcher == nil {
			slog.Info("No event watcher (expected for PostgreSQL)")
			// For PostgreSQL, just wait for context cancellation
			<-ctx.Done()
			slog.Info("Event watcher goroutine exiting (PostgreSQL mode)")
